...
QList<int> Config::GetLikeMusicId()
{
	//QSettings setting("../config/MyLikeMusicId.ini", QSettings::IniFormat);
	m_psetting->beginGroup("MusicId");
	if (!m_psetting->allKeys().isEmpty()) {
		MusicId.clear();
		int len = m_psetting->allKeys().length();
		foreach(const QString& rhs, m_psetting->allKeys()) {
			MusicId.append(m_psetting->value(rhs, "null").toUInt());
		}
		m_psetting->endGroup();
		return MusicId;
	}
